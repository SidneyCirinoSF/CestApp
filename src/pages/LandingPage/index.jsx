...
export default function LandingPage() {
  ...
  const images = ["/images/doacao.png", "/images/estoque.jpg", "/images/funcionarios.jpg"]; // Substitua pelos caminhos reais

  const nextSlide = () => setCurrentSlide((prev) => (prev + 1) % images.length);
  const prevSlide = () => setCurrentSlide((prev) => (prev - 1 + images.length) % images.length);

  useEffect(() => {
    const interval = setInterval(nextSlide, 5000); // Troca automática a cada 5 segundos
